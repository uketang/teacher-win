 ...
 * @example
 *     client.call('getSomeData', null, (response)=>{});
 **/
uimsdk.Client.prototype.call = function(name, args, callback){
    if (!name || !callback) {
        throw Error('Invalid arguments, name and callback required.');
    };
    return this.__call__(name.toString(), args.toString(), callback);
}
 ...
 * @example
 *     var msg = client.getMessageById(msgIds);
 **/
 ...
 * @param msgIds {Array}  array of message id. 
 * @example
 *     var count = client.setMessageRead(msgIds);
 **/
 ...
 * 获取小于指定id的message数据。
 *
 * @method getMessageBefore
 * @public
 * @sync
 * @param startId {Number} start id of message. 
 * @param pageSize {Number} page size of message to return. 
 * @param type {Number} type of message to return(all : -1). 
 * @return message {Array} array of messages
 * @example
 *     var msgs = client.getMessageBefore(startId, pageSize);
 **/
uimsdk.Client.prototype.getMessageBefore = function(startId, pageSize, type){
    if (typeof startId !== 'number' || typeof pageSize !== 'number') {
        throw Error('Invalid arguments, start messageid and page size required.');
    };
    return this.__getMessageBefore__(startId, pageSize, type);
}
